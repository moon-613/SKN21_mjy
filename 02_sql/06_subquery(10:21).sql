...
- 어느 구절에 사용되었는지에 따른 구분
    ...
    - 인라인 뷰 - from 절에 사용되어 테이블의 역할.
    
- 서브쿼리 조회결과 행수에 따른 구분
    - 단일행 서브쿼리 - 서브쿼리의 조회결과 행이 한 행인 것.
    - 다중행 서브쿼리 - 서브쿼리의 조회결과 행이 여러행인 것.

- 동작 방식에 따른 구분 (밖이 메인쿼리 안에 서브쿼리)
    - 비상관 서브쿼리 - 서브쿼리에 메인쿼리의 컬럼이 사용되지 않는다.
                   메인쿼리에 사용할 값을 서브쿼리가 제공하는 역할.
    - 상관 서브쿼리 - 서브쿼리에서 메인쿼리의 컬럼을 사용. 
...
select dept_id, dept_name, concat('$ ', format(급여평균, 2)) "급여평균"
from (
...
-- select * from emp where mgr_id = (select * from emp where emp_name = 'Alexander'); -- 2행이 조회되어 에러 발생.
...
select * from emp 
where salary > (select max(salary) from emp where emp_id in (101, 102, 103));  -- max 이용. 위와 같음.
...
select * from job where job_id in (select job_id from job where max_salary <= 6000);
