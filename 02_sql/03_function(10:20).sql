...
- 문자열관련 함수
 ...
 ltrim(문자열), rtrim(문자열), trim(문자열): 문자열에서 왼쪽(ltrim), 오른쪽(rtrim), 양쪽(trim)의 공백 제거. 중간공백은 유지
...
select emp_name, lpad(emp_name, 15,' ') as "emp_name" from emp;
...
- 숫자관련 함수
 ...
 ceil(값): 값보다 큰 정수 중 가장 작은 정수. 소숫점 이하 올린다. 
 floor(값): 값보다 작은 정수 중 가장 작은 정수. 소숫점 이하를 버린다. 내림
...
-- select round(12345.12345, 0)  # 자리 0 이하에서 반올림 (정수)

# x  x  x  x  x.xxxxx
# -4 -3 -2 -1 0. 12345

-- EMP 테이블에서 각 직원에 대해 직원ID(emp_id), 이름(emp_name), 급여(salary) 그리고 15% 인상된 급여(salary)를 조회하는 질의를 작성.
...
-- MP 테이블에서 부서이름(dept_name)이 'IT'인 직원들의 '입사일(hire_date)로 부터 10일전', 입사일, '입사일로 부터 10일 후' 의 날짜를 조회. 
...
select emp_name, timestampdiff(month, hire_date, curdate()) "근무 개월 수"
from emp
order by 2 desc;
...
-- 20000 이상, 20000~10000, 10000 미만
       
select salary, case when salary >= 20000 then "1등급"
