...
- distinct 컬럼명 => 중복된 결과를 제거.

참고: 
- Sql은 대소문자 구분 안 함.
- sql문 실행: control + enter (이전 ; 부터 다음 ; 까지 실행)
*************************************** */

-- EMP 테이블의 모든 컬럼의 모든 항목을 조회 (전직원 정보 조회).
select * from testdb.emp;

use testdb;  # testdb에 있는 거 사용할 거라는 뜻.
select * from emp;
...
select distinct job from emp;  # 범주형 데이터 타입일 경우 확인.
...
select emp_id as 직원ID,  # 'emp_id as 직원ID' 하면 emp_id 조회결과를 직원ID 컬럼으로 보이라는 뜻.
       emp_name as 직원이름,  # 'upper(emp_name)' 하면 대문자로 나옴.
       hire_date as 입사일,  # as는 생략 가능.
       salary as 급여,
       dept_name as 소속부서  # 소속 부서 이렇게 띄어쓰기 할 경우 "소속 부서" 이렇게 ""쌍따옴표 넣기.
from   emp;
...
- 연산은 그 컬럼의 모든 값들에 일률적으로 적용.
- 같은 컬럼을 여러번 조회할 수 있다.
************************************** */

select salary, salary *12 from emp;  # 여러 번 조회 가능.
...
select emp_name, concat ('$ ', salary) as salary from emp;
...
/* *************************************
where 절을 이용한 행 선택. where절 연습 많이 하세용~
...
# where comm_pct between 0.2 and 0.3  # 위와 같음.
# where comm_pct not between 0.2 and 0.3;  # 0.2와 0.3 사이가 아닌 값.
...
# where job in ('IT_PROG', 'ST_MAN'); 위와 같음. 
# where job not in ('IT_PROG', 'ST_MAN');  # 아닌 것.


-- EMP 테이블에서 직원 이름(emp_name)이 S로 시작하는 직원의  ID(emp_id), 이름(emp_name)을 조회.

select emp_id, emp_name from emp 
where emp_name like 'S%';   # % = 글자 수와 문자가 뭐가 오든 상관없다. 
# where emp_name like '%en';  # en으로 끝나는 이름인 경우 조회.
# where emp_name like '%are%';  # ~를 포함한 이름인 경우 조회.
# where emp_name like 'A%B';  # A로 시작하고 B로 끝나는 이름인 경우 조회.

-- EMP 테이블에서 직원 이름(emp_name)의 세 번째 문자가 “e”인 모든 사원의 이름을 조회

select emp_name from emp where emp_name like '__e%';   # '_' : 한 글자.

-- EMP 테이블에서 직원의 이름에 '%' 가 들어가는 직원의 ID(emp_id), 직원이름(emp_name) 조회
--    %나 _ 를 검색하는 값으로 사용할 경우. 

select emp_name from emp where emp_name like '%&%%' escape '&';
# escape '특수문자'  특수문자 뒤에 %나 _는 조회할 값임을 표시. (like의 패턴문자가 아니라)


-- EMP 테이블에서 부서명(dept_name)이 null인 직원의 ID(emp_id), 이름(emp_name), 부서명(dept_name) 조회.
...
# where year(hire_date) = 2004;  # 위와 같음.
# where hire_date > '2004-05-20';  # '2004-05-20' 이후 입사한 직원 조회.
# select year(hire_date) from emp; 하면 year만 빼서 줌.
 ...
 *******************************************/
 ...
 # 넘 재밌어서 쉬는 시간에 푼 문제들. 다 맞았다 !!!! 매일매일이 이랬으면 좋겠다.
 
-- EMP 테이블에서 'SA_REP' 업무를 담당하는 직원들 중 급여(salary)가 $9,000인 직원들을 조회.

select * from emp where job = 'SA_REP' and salary = 9000;

-- EMP 테이블에서 업무(job)가 'FI_ACCOUNT' 거나 급여(salary)가 $8,000 이상인 직원들 조회.

select * from emp where job = 'FI_ACCOUNT' or salary >= 8000;

-- EMP 테이블에서 업무(job)에 'MAN'이 들어가는 직원들 중 부서(dept_name)가 'Shipping' 이고 2005년이후 입사한 직원들 조회.

select * from emp where job like '%MAN%' and dept_name = 'Shipping' and hire_date >= '2005-01-01';

-- EMP 테이블에서 업무(job)에 'MAN'이 들어가는 직원들 중 'Marketing' 이나 'Sales' 부서에 소속된 직원들의 ID(emp_id), 이름(emp_name), 업무(job), 부서(dept_name) 조회. 
...
- order by 정렬기준컬럼 정렬방식 [, ...]
    - 정렬기준컬럼 지정 단위: 컬럼이름, 컬럼의 순번(select절의 선언 순서)
     `select salary, hire_date from emp` 에서 salary 컬럼을 기준으로 정렬할 경우 `order by salary` 또는 `order by 1` 로 작성 가능.
	 
    - 정렬방식
        - ASC : 오름차순(ascending), 기본방식(생략 가능)
