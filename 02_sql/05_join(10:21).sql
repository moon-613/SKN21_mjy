...
/* ********************************************************************************
조인(JOIN) 
: 2개 이상의 테이블에 있는 컬럼들을 합쳐서 가상의 테이블을 만들어 조회하는 방식.
 	- 소스테이블 : 내가 먼저 읽어야 한다고 생각하는 테이블
	- 타겟테이블 : 소스를 읽은 후 소스에 조인할 대상이 되는 테이블
 
- join 연산: 각 테이블을 어떻게 합칠지를 표현하는 것.
    - 조인 연산에 따른 조인종류
        - Equi join , non-equi join
- 조인의 종류
    - Inner Join 
        - 양쪽 테이블에서 조인 조건 만족하는 행들만 합친다. 
    - Outer Join
        - 한 쪽 테이블의 행들을 모두 사용하고 다른 쪽 테이블은 조인 조건을 만족하는 행만 합친다. 조인조건을 만족하는 행이 없는 경우 NULL을 합친다.
...
-- A B
-- from A INNER JOIN B ON A.a > B.b  # INNER JOIN에서 INNER 생략 가능.
...
from emp e join dept d on e.dept_id = d.dept_id;  -- emp 대신 e 쓸게. INNER 생략한 것.
...
select e.emp_id, e.emp_name, e.salary, concat(s.grade, '등급') "급여 등급"
...
select d.dept_id, d.dept_name, format(avg(e.salary), 2) "급여평균"  -- format 쓰면 문자열로 바뀜. 9,8,7, ...
from emp e join dept d on e.dept_id = d.dept_id
...
from emp e join job j on e.job_id = j.job_id
           join salary_grade s on e.salary between s.low_sal and high_sal
           join dept d on e.dept_id = d.dept_id = d.dept_id;
...
from emp e left outer join dept d on e.dept_id = d.dept_id  -- left outer join = left join. outer 생략 가능.
order by 4 desc;
...
-- group by로 나눠서 직원 수 구하라는 거죠~

select d.dept_id, d.dept_name, count(emp_id) "직원 수"  -- 직원 수 -> 내가 세려고 하는 테이블의 PK(Primary Key)로 count
...
select e.emp_id, e.emp_name "직원 이름", m.emp_name "상사 이름", date_format(e.hire_date, '%Y/%m/%d') "hire_date"
...
from emp e left join job j on e.job_id = j.job_id
		   ...
		   left join emp m on m.mgr_id = m.emp_id    -- m: 상사, e: 직원
where year(e.hire_date) between 2003 and 2005;



select e.emp_id, d.*,  -- dept의 모든 컬럼을 조회.
       j.job_title, m.emp_name "상사 이름"
from emp e left join job j on e.job_id = j.job_id
           left join dept d on e.dept_id = d.dept_id
		   left join emp m on m.mgr_id = m.emp_id
where year(e.hire_date) between 2003 and 2005;
