...
- count(*) 를 제외한 모든 집계함수들은 null을 제외하고 집계. 
...
select max(hire_date)"가장 최근 입사일", min(hire_date) "가장 오래된 입사일" from emp;

-- EMP 테이블의 부서(dept_name) 의 개수를 조회

select count(dept_name) from emp;
select count(distinct dept_name) from emp;
select count(distinct ifnull(dept_name, 'a')) from emp;  -- null인 것도 포함해서 count 할 경우.
...
- 구문
  - group by 컬럼명 [, 컬럼명]
    - 컬럼명
      - 집계를 위해 group으로 묶어줄 기준 컬럼명을 지정.
      - select절에 기준컬럼을 지정한 경우 컬럼 순번(1부터 시작)으로 지정 가능.
      - 지정한 기준 컬럼(들)이 같은 값을 가지는 행들이 같은 그룹으로 묶인다.
      - 같은 그룹으로 묶인 행들의 값을 기준으로 집계한다.
      - 기준 컬럼은 범주형 컬럼을 사용. 부서별 급여 평균 => 부서컬럼, 성별 급여 합계 => 성별컬럼
