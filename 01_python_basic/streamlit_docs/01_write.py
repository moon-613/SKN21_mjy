...
## OS 지원 이모지: `window키 + .` (맥:ctrl + command + space 선택)
st.title('즐겁게 합시다. :laughing:')

# Header 입력
st.header('헤더(중제목)를 입력할 수 있어요! :star2:')

# Subheader 입력
st.subheader('이것은 subheader(소제목) 입니다 :100:')
...
## 가변인자로 출력할 내용들을 나열해서 전달하면 나열되어 출력된다. 자주 사용.
st.write("나이:", str(26), "이름:", "문지영")
st.write(1, 2, 3, 4, 5)  
st.write(3.22, 5e-2)
st.write(True, False)
...
st.success("성공")  # 초록
st.info("정보출력")  # 파랑
st.warning(":warning: 경고 :warning:")  # 노랑
st.error("에러")  # 빨강
