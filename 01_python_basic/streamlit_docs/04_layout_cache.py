...
col1, col2 = st.columns(2)  # 한 줄에 2개 열 배치
# print(type(col1))
col11, col12 = col1.columns(2)  # col1을 다시 2개 열로 나눔.
col11.title("제목")  
col11.header("중제목")
col11.subheader("소제목")
col11.text("일반 글1")
...
@st.cache_data   # a.txt 파일을 읽어서 문자열로 반환하는 함수.
def get_txt():
    f = open("a.txt")
    return f.read()   # (str) 값을 반환하는 함수.

@st.cache.resource   # a.txt 파일을 읽어서 파일 객체로 반환하는 함수.
def get_txt_stream():
    f = open('a.txt')
    return f
...
@st.cache_data  # 한 번 호출 후 캐시된 데이터를 반환. (재호출 안함)
def get_data():
    print("get_data")
