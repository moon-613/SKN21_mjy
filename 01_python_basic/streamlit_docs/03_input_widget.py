...
st.set_page_config(page_title="Input Widget", layout="wide") # 인터넷 타이틀 설정. wide: 화면 꽉참
...
if name_value:  # 한 글자라도 있을 때 "이름: " 출력.
...
st.subheader("링크버튼")
col1, col2, col3 = st.columns(3)  # 한 줄에 3개 버튼 배치
...
option = st.selectbox(
    "지역을 선택하세요",
    ("서울", "인천", "부산", "광주", "대구"),
    # index=None  # 기본 선택값 지정. (0부터 시작하는 index값)
)
...
@st.cache_data
def get_data():
    # csv 파일을 읽어서 DataFrame(pandas의 표)으로 생성 (read_csv).
    # 앞 10개 행만 반환. (head(10))
    df = pd.read_csv("data/boston_housing.csv").head(10)  # csv: text로 된 표 데이터.
    return df
...
uploaded_file = col4.file_uploader(
    ...
    accept_multiple_files=False    # Fasle: 한 번에 한 개. True 설정 시 한번에 여러개 파일 업로드 가능.
)
